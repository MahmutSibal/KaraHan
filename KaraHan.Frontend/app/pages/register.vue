...
async function submit() {
  ...
  try {
    ...
    error.value = 'Kayit basarisiz. Lutfen tekrar deneyin.'
  }
  finally {
    pending.value = false
  }
  ...
  <main class="auth-wrap kh-appear">
    <form class="auth-card kh-panel" @submit.prevent="submit">
      ...
      <button :disabled="pending" type="submit" class="kh-btn kh-btn-primary">
        {{ pending ? 'Bekleyin...' : 'Kayit Ol' }}
      </button>

      <NuxtLink to="/login" class="switch-link">Zaten hesabin var mi? Giris yap</NuxtLink>
    </form>
  </main>
</template>
